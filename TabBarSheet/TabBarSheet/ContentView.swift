...
struct ContentView: View {
    ...
    var body: some View {
        // @Bindable 会导致 PreView 崩溃，直接上模拟器
                ...
                ScrollView {
                    ...
                    VStack(spacing: 15) {
                        if windowSharedModel.activeTab == .devices {
                            DevicesRowView("iphone", "iJustine's iPhone", "Home")
                            DevicesRowView("ipad", "iJustine's iPad", "Home")
                            DevicesRowView("applewatch", "iJustine's applewatch", "Home")
                        }
                    }
...
//            sceneDelegate.addTabBar(windowSharedModel)
    ...
    @ViewBuilder
    func DevicesRowView(_ image: String, _ title: String, _ subTitle: String) -> some View {
        HStack(spacing: 12) {
